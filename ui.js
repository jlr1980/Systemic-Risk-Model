// ============================================================
// SYSTEMIC RISK MODEL v4.0 - UI CONTROLLER
// ============================================================
...
document.querySelectorAll('.tab').forEach(t => {
  t.addEventListener('click', () => {
    ...
    if (results) setTimeout(resizeCharts, 50);
  });
});
...
function updateAll() {
  ...
  // Cascade tab
  const casP = ((pct[3] + pct[4]) * 100).toFixed(1);
  const casEl = document.getElementById('cascade-prob');
  casEl.textContent = casP + '%';
...
function drawTimeline() {
  ...
  makeChart('chart-timeline', {
    ...
    options: {
      ...cDef,
      plugins: { ...cDef.plugins, tooltip: { mode: 'index', intersect: false } },
      scales: {
        ...cDef.scales,
        y: { ...cDef.scales.y, stacked: true, min: 0, max: 100, ticks: { ...cDef.scales.y.ticks, callback: v => v + '%' } },
        x: { ...cDef.scales.x, stacked: true }
      },
...
function drawPaths() {
  const labels = Array.from({length:11}, (_,i) => 2026+i);
  const datasets = results.samplePaths.slice(0, 80).map((p, i) => ({
    data: p,
    borderColor: ENGINE.STATE_COLORS[p[p.length-1]] + '30',
    borderWidth: 0.8,
    pointRadius: 0,
    tension: 0.2,
  }));
  makeChart('chart-paths', {
    ...
    options: {
      ...
      scales: {
        ...cDef.scales,
        y: { ...cDef.scales.y, min: -0.3, max: 4.3, ticks: { ...cDef.scales.y.ticks, callback: v => ENGINE.STATE_SHORT[Math.round(v)] || '' } }
      }
    }
  });
}
  ...
  makeChart('chart-substrate', {
    ...
    data: {
      ...
      datasets: [{
        ...
        data: data,
        borderColor: '#0d9da0',
        backgroundColor: 'rgba(13,157,160,0.1)',
        fill: true,
        borderWidth: 2,
        pointRadius: 3,
        pointBackgroundColor: data.map(v => v > 60 ? '#3a9e6e' : v > 35 ? '#c49a40' : '#c44040'),
        tension: 0.3,
      },{
        label: 'Critical Threshold',
        data: new Array(11).fill(50),
  ...
  makeChart('chart-cascade', {
    ...
    data: {
      ...
      datasets: [{
        ...
        borderColor: ['#c49a40','#c49a40','#c44040','#c44040','#c44040'],
        borderWidth: 1,
      }]
    },
    options: {
      ...
      scales: {
        ...cDef.scales,
        x: { ...cDef.scales.x, min: 0, max: 100, ticks: { ...cDef.scales.x.ticks, callback: v => v + '%' } }
      }
    }
  });
}
...
function drawTrajectory() {
  ...
  makeChart('chart-trajectory', {
    ...
    data: {
      labels,
      datasets: [
        { label: 'Conventional (10-90th)', data: r.percentilePaths.conv10, borderColor: 'transparent', backgroundColor: 'rgba(196,64,64,0.08)', fill: '+1', pointRadius: 0 },
        ...
        { label: 'Formation (10-90th)', data: r.percentilePaths.form10, borderColor: 'transparent', backgroundColor: 'rgba(13,157,160,0.08)', fill: '+1', pointRadius: 0 },
    ...
    options: {
      ...
      plugins: {
        ...cDef.plugins,
        legend: { display: true, labels: { color: '#7a8fa0', font: { family: "'DM Sans'", size: 10 }, filter: item => item.text && item.text.length > 0 && !item.text.includes('10-90') } }
      },
      scales: {
        ...cDef.scales,
        y: { ...cDef.scales.y, ticks: { ...cDef.scales.y.ticks, callback: v => '$' + (v).toFixed(0) + 'M' } }
      }
    }
  });
}
...
function drawPortfolioDist() {
  ...
  const labels = [];
  const dC = new Array(bins).fill(0);
  const dR = new Array(bins).fill(0);
  const dF = new Array(bins).fill(0);
  for (let i = 0; i < bins; i++) labels.push('$' + (min + i * bw).toFixed(0) + 'M');

  const binIt = (arr, d) => {
    for (const v of arr) {
      const vi = v / 100 * 100; // convert from base-100 to $M
      const b = Math.min(bins - 1, Math.max(0, Math.floor((vi - min) / bw)));
      d[b]++;
    }
    const n = arr.length;
    for (let i = 0; i < bins; i++) d[i] = (d[i] / n) * 100;
  };
  binIt(r.portfolios.conv, dC);
  binIt(r.portfolios.real, dR);
  binIt(r.portfolios.form, dF);
  ...
  makeChart('chart-portfolio', {
    ...
    options: {
      ...
      plugins: {
        ...cDef.plugins,
        legend: { display: true, labels: { color: '#7a8fa0', font: { family: "'DM Sans'", size: 10 } } }
      },
      scales: {
        ...cDef.scales,
        x: { ...cDef.scales.x, ticks: { ...cDef.scales.x.ticks, maxTicksLimit: 10 } },
        y: { ...cDef.scales.y, ticks: { ...cDef.scales.y.ticks, callback: v => v.toFixed(0) + '%' } }
      }
    }
  });
}
...
function buildCorrMatrix() {
  ...
  const lookup = {};
  for (const [a, b, m] of ENGINE.CORRELATIONS) {
    lookup[a + '|' + b] = m;
    lookup[b + '|' + a] = m;
  }
  ...
  for (let r = 0; r < n; r++) {
    ...
    for (let c = 0; c < n; c++) {
      if (r === c) {
        html += `<div class="corr-c dg">-</div>`;
      } else {
        const key = ids[r] + '|' + ids[c];
        const v = lookup[key];
        if (v) {
          const cls = v >= 1.4 ? 'hi' : v >= 1.2 ? 'md' : '';
          html += `<div class="corr-c ${cls}">${v.toFixed(1)}x</div>`;
        } else {
          html += `<div class="corr-c">1.0</div>`;
        }
      }
    }
  }
...
function buildMethodTable() {
  ...
  for (const evt of ENGINE.EVENTS) {
    ...
    tr.innerHTML = `
      ...
      <td class="mono">${evt.base}%</td>
      <td class="mono">Y${evt.peak[0]}-${evt.peak[1]}</td>
      <td class="mono">${evt.mag.toFixed(2)}</td>
      <td class="mono">${evt.dur}yr</td>
      <td style="font-size:0.65rem;color:var(--text2)">${src}</td>
    `;
    body.appendChild(tr);
  }
}
